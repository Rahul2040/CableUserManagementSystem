        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(!cid.equals("")&&!smc.equals("")&&!ob.equals("")&&!pack.equals("")&&!add.equals("")&&!acc.equals("")){
                    cid=checkcusid(cid);
                    if(name.equals("")) name="!";
                    cusinfo cus= new cusinfo(cid,name,smc.toUpperCase().replaceAll("E","Y").replaceAll(" ",""),Integer.parseInt(ob),Integer.parseInt(pack),Integer.parseInt(add),acc);
                    FirebaseFirestore db= FirebaseFirestore.getInstance();
                    final CollectionReference noteref= db.collection("List");
                    noteref.add(cus).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                        @Override
